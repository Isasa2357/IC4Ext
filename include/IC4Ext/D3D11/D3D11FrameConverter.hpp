...
public:
    ...
    // Compatibility entry point kept for older callers. New code should use the
    // D3D11Core overload so resource/view/pipeline creation goes through D3D11Helper.
...
private:
    bool ensurePipeline(CameraPixelFormat input,
                        ...
                        bool& ready);
    bool loadShaderBytecode(const std::string& baseName, D3D11CoreLib::ShaderBytecode& outBytecode);
    bool createRawInputBuffer(const CpuFrameView& input,
    ...
    bool createOutputTexture(const CpuFrameView& input,
                             ...
                             D3D11CameraFrame& outFrame);
    bool createConstantBuffer(const CpuFrameView& input, D3D11CoreLib::D3D11Resource& buffer);
