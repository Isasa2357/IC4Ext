...
class D3D11DummyCameraCapture final : public ID3D11Camera
{
public:
    D3D11DummyCameraCapture(std::uint32_t cameraIndex,
                     std::shared_ptr<D3D11FrameQueue> frameQueue,
                     std::weak_ptr<ID3D11CameraControlSink> controlSink);

    ~D3D11DummyCameraCapture() override = default;

    D3D11DummyCameraCapture(const D3D11DummyCameraCapture&) = delete;
    D3D11DummyCameraCapture& operator=(const D3D11DummyCameraCapture&) = delete;

    std::uint32_t cameraIndex() const noexcept { return cameraIndex_; }
    std::shared_ptr<D3D11FrameQueue> frameQueue() const noexcept { return frameQueue_; }
...
private:
    ...
    std::shared_ptr<D3D11FrameQueue> frameQueue_;
    std::weak_ptr<ID3D11CameraControlSink> controlSink_;
