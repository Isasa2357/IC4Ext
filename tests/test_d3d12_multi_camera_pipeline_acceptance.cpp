...
struct Settings
{
    ...
    std::uint64_t toleranceNs = 10'000'000;
    int warmupSets = 20;
    int targetSets = 100;
...
struct Measurement
{
    ...
    std::chrono::steady_clock::time_point firstMeasuredTime{};
    std::chrono::steady_clock::time_point lastMeasuredTime{};
};
...
Settings LoadSettings()
{
    ...
    case TestMode::FreeRun:
        ...
        settings.toleranceNs = 10'000'000;
    ...
    settings.warmupSets = std::max(
        0, IC4ExtTest::EnvInt("IC4EXT_TEST_WARMUP_SETS", settings.warmupSets));
    settings.targetSets = std::max(
        1, IC4ExtTest::EnvInt("IC4EXT_TEST_SYNC_SETS", settings.targetSets));
...
bool IsManualModeEnabled(TestMode mode)
{
    if (mode == TestMode::HardwareTrigger) {
        return IC4ExtTest::EnvInt(
                   "IC4EXT_TEST_ENABLE_HW_ACCEPTANCE",
                   0) != 0;
    }
    if (mode == TestMode::LongRun160Fps) {
        return IC4ExtTest::EnvInt(
                   "IC4EXT_TEST_ENABLE_LONG_RUN_ACCEPTANCE",
                   0) != 0;
    }
    return true;
}
...
std::uint64_t HostTimestampNs(const Pipe::ReadOnlyFrame& frame)
{
    ...
    if (time == std::chrono::steady_clock::time_point{}) return 0;
...
bool StopStartedGroup(Pipe::MultiCameraStartupResult& started) noexcept
{
    ...
    for (auto& thread : started.captureThreads) {
        if (thread) {
            stopped = thread->stopAcquisition() && stopped;
        }
    }
    ...
    const StatsSnapshot& current)
{
    ...
    if (measurement.firstMeasuredTime !=
    ...
    std::cerr << std::fixed << std::setprecision(3)
              << "[v2-acceptance] mode=" << ModeName(settings.mode)
    ...
    std::string& failure)
{
    ...
    if (measurement.haveLastFrameNumbers) {
        if (frame0->timing().frameNumber <= measurement.lastFrame0 ||
    ...
    std::string& failure)
{
    if (measurement.sets == 0 ||
        measurement.firstMeasuredTime ==
            std::chrono::steady_clock::time_point{} ||
        measurement.lastMeasuredTime ==
            std::chrono::steady_clock::time_point{}) {
    ...
    if (outputDrops != 0) {
        failure = "FrameSyncThread output queue dropped synchronized sets";
        return false;
    }
    ...
    if (settings.mode == TestMode::LongRun160Fps) {
        ...
        const std::uint64_t allowedImbalance =
...
int main()
{
    ...
    std::cerr << "[v2-acceptance] mode=" << ModeName(settings.mode)
    ...
    if (!IsManualModeEnabled(settings.mode)) {
        std::cerr
            << "Skipping manual hardware acceptance mode. Set "
            << (settings.mode == TestMode::LongRun160Fps
                    ? "IC4EXT_TEST_ENABLE_LONG_RUN_ACCEPTANCE=1"
                    : "IC4EXT_TEST_ENABLE_HW_ACCEPTANCE=1")
            << " to run it.\n";
        return 77;
    }
    ...
    const bool hardwareTriggered =
        settings.mode != TestMode::FreeRun;
    if (hardwareTriggered) {
        IC4Ext::ConfigureHardwareTriggerSync(
            config0, settings.triggerSource);
        IC4Ext::ConfigureHardwareTriggerSync(
            config1, settings.triggerSource);
    } else {
    ...
    std::cerr
        << "[v2-acceptance] starting two CameraCaptureThread instances "
    ...
    if (hardwareTriggered && settings.triggerArmDelayMs > 0) {
        std::cerr
            << "[v2-acceptance] cameras are armed. Start the external "
               "hardware trigger now; collection begins in "
    ...
    const double nominalTargetSeconds =
        settings.expectedFps > 0.0
            ? static_cast<double>(settings.targetSets) / settings.expectedFps
            : static_cast<double>(settings.startupTimeoutSeconds);
    const int collectionTimeoutSeconds =
        settings.mode == TestMode::LongRun160Fps
            ? settings.acceptanceSeconds + 30
            : std::max(
                  30,
                  static_cast<int>(std::ceil(nominalTargetSeconds * 3.0)) + 10);
    const auto overallDeadline =
        loopStart +
        std::chrono::seconds(
            settings.startupTimeoutSeconds + collectionTimeoutSeconds);
    auto nextProgress =
        loopStart + std::chrono::seconds(5);
    ...
    while (true) {
        ...
        if (completedWarmupSets < settings.warmupSets) {
            ...
            if (completedWarmupSets == settings.warmupSets) {
                ...
                measurement = {};
                std::cerr
                    << "[v2-acceptance] warmup complete; measurement begins\n";
            }
            continue;
        }
        ...
        if (settings.mode != TestMode::LongRun160Fps &&
            measurement.sets >=
                static_cast<std::uint64_t>(settings.targetSets)) {
            break;
        }
    }
    ...
    std::cerr << "[v2-acceptance] stopping acquisitions and workers\n";
    const bool acquisitionsStopped = StopStartedGroup(started);
    sync.stopAndJoin();
    core->WaitIdle();

    if (!failure.empty()) {
        std::cerr << "[v2-acceptance] FAILED: " << failure << '\n';
        return 1;
    }
    if (!acquisitionsStopped) {
        std::cerr
            << "[v2-acceptance] FAILED: one or more AcquisitionStop commands failed\n";
        return 1;
    }
    if (!baselineCaptured) {
        std::cerr << "[v2-acceptance] FAILED: no measurement baseline\n";
        return 1;
    }
    ...
    std::cout << "test_d3d12_multi_camera_pipeline_acceptance passed"
              << " mode=" << ModeName(settings.mode) << '\n';
    return 0;
}
