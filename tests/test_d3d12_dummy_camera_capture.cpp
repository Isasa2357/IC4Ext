...
class MockControlSink final : public IC4Ext::ID3D12CameraControlSink
{
public:
    bool result = true;
    ...
    bool submitControlCommand(const IC4Ext::CameraControlCommand& command) override
    {
        ...
        error = result ? IC4Ext::NoError()
                       : IC4Ext::MakeError(IC4Ext::ErrorCode::D3D12Error, "MockControlSink", "forced failure");
        return result;
    }
...
int main()
{
    ...
    auto queue = std::make_shared<IC4Ext::D3D12FrameQueue>(queueOptions);

    {
        IC4Ext::D3D12DummyCameraCapture noSink(3, queue, std::weak_ptr<IC4Ext::ID3D12CameraControlSink>{});
    ...
    IC4Ext::D3D12DummyCameraCapture dummy(7, queue, sink);
    ...
    assert(dummy.lastError().code == static_cast<int>(IC4Ext::ErrorCode::D3D12Error));
    ...
    IC4Ext::D3D12ReadResult read = dummy.read(readOptions);
    assert(read.ok);
    ...
    std::cout << "test_d3d12_dummy_camera_capture passed\n";
    return 0;
}
