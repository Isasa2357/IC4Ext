...
IC4Ext::D3D11::FrameSyncTimestampSource ParseTimestampSource(
    const std::string& text)
{
    if (text == "host" || text == "HostReceived") {
        return IC4Ext::D3D11::FrameSyncTimestampSource::HostReceived;
    }
    if (text == "device" || text == "Device") {
        return IC4Ext::D3D11::FrameSyncTimestampSource::Device;
    }
    return IC4Ext::D3D11::FrameSyncTimestampSource::Auto;
}

const char* ToString(
    IC4Ext::D3D11::FrameSyncTimestampSource source) noexcept
{
    switch (source) {
    case IC4Ext::D3D11::FrameSyncTimestampSource::HostReceived:
        return "host";
    case IC4Ext::D3D11::FrameSyncTimestampSource::Device:
        return "device";
    case IC4Ext::D3D11::FrameSyncTimestampSource::Auto:
    default:
        return "auto";
    }
}
...
std::uint64_t AbsDiff(std::uint64_t lhs, std::uint64_t rhs) noexcept
{
...
IC4Ext::CameraCaptureConfig MakeCameraConfig(
    ...
    const char* jsonDeviceIndexArgument)
{
    ...
    const char* jsonPath = ArgValue(argc, argv, jsonArgument);
    const int jsonDeviceIndex = ArgInt(
        argc,
        argv,
        jsonDeviceIndexArgument,
        0);
    ...
    config.streamRequest.requestedFormat = ParseCameraFormat(
        ArgValue(argc, argv, "--format")
            ? ArgValue(argc, argv, "--format")
            : "BGR8");
    config.streamRequest.forceRequestedFormat =
        HasArg(argc, argv, "--force-format");
...
struct DrainStats
{
    ...
    IC4Ext::D3D11::SyncGroupId lastSyncGroupId = 0;
    std::uint64_t lastFrame0 = 0;
    std::uint64_t lastFrame1 = 0;
};

void DrainPairQueue(
    const std::shared_ptr<IC4Ext::D3D11::ReadOnlyFrameSetQueue>& queue,
    DrainStats& stats)
{
    for (;;) {
        ...
        if (frame0 && frame1 &&
            ...
            const auto difference = AbsDiff(
                frame0->timing().deviceTimestampNs,
                frame1->timing().deviceTimestampNs);
            ++stats.diffSamples;
            stats.sumDeviceDiffNs += difference;
            stats.maxDeviceDiffNs = std::max(
                stats.maxDeviceDiffNs,
                difference);
        }
    }
}
...
{
    const double pairFps = elapsedSeconds > 0.0
        ? static_cast<double>(stats.pairSets) / elapsedSeconds
        : 0.0;
    const double camera0Fps = elapsedSeconds > 0.0
        ? static_cast<double>(stats.camera0Sets) / elapsedSeconds
        : 0.0;
    const double camera1Fps = elapsedSeconds > 0.0
        ? static_cast<double>(stats.camera1Sets) / elapsedSeconds
        : 0.0;
    const double meanDifferenceUs = stats.diffSamples > 0
        ? static_cast<double>(stats.sumDeviceDiffNs) /
              static_cast<double>(stats.diffSamples) / 1000.0
        : 0.0;
    ...
    std::cout << prefix
              ...
              << " meanDeviceDiffUs=" << meanDifferenceUs
              << " maxDeviceDiffUs="
              << static_cast<double>(stats.maxDeviceDiffNs) / 1000.0
              << std::endl;
}
...
int main(int argc, char** argv)
{
    ...
    const int durationSeconds = ArgInt(argc, argv, "--duration-sec", 10);
    const int reportMilliseconds = ArgInt(argc, argv, "--report-ms", 1000);
    const int readTimeoutMilliseconds =
        ArgInt(argc, argv, "--read-timeout-ms", 1000);
    ...
    const int poolMaximum = ArgInt(argc, argv, "--pool-max", 32);
    const double singleOutputFps =
        ArgDouble(argc, argv, "--single-output-fps", 30.0);
    const auto timestampSource = ParseTimestampSource(
        ArgValue(argc, argv, "--timestamp-source")
            ? ArgValue(argc, argv, "--timestamp-source")
            : "auto");
    const auto maxDifferenceUs = static_cast<std::uint64_t>(
        std::max(1, ArgInt(argc, argv, "--max-diff-us", 4000)));
    ...
    auto config0 = MakeCameraConfig(
        argc,
        argv,
        "--ic4-json0",
        "--json-device-index0");
    auto config1 = MakeCameraConfig(
        argc,
        argv,
        "--ic4-json1",
        "--json-device-index1");
    ...
    captureOptions.maxFramePoolCapacity =
        static_cast<std::size_t>(std::max(poolInitial, poolMaximum));

    Pipe::CameraCaptureThreadOptions threadOptions;
    threadOptions.readTimeoutMs = static_cast<std::uint32_t>(
        std::max(1, readTimeoutMilliseconds));
    threadOptions.stopOnReadError = false;

    ThreadKit::Queues::QueueOptions ingressOptions;
    ingressOptions.maxSize = static_cast<std::size_t>(
        std::max(1, inputQueueSize));
    ...
    syncConfig.maxTimestampDiffNs = maxDifferenceUs * 1000ull;
    ...
    outputOptions.maxSize = static_cast<std::size_t>(
        std::max(1, outputQueueSize));
    outputOptions.overflowPolicy =
        ThreadKit::Queues::QueueOverflowPolicy::DropOldest;
    auto pairQueue = std::make_shared<Pipe::ReadOnlyFrameSetQueue>(
        outputOptions);
    auto camera0Queue = std::make_shared<Pipe::ReadOnlyFrameSetQueue>(
        outputOptions);
    auto camera1Queue = std::make_shared<Pipe::ReadOnlyFrameSetQueue>(
        outputOptions);
    ...
    const auto camera0OutputId = syncThread.registerOutput(
        camera0Queue,
        camera0Output);

    Pipe::FrameSyncOutputConfig camera1Output = camera0Output;
    camera1Output.requiredCameras = {1};
    const auto camera1OutputId = syncThread.registerOutput(
        camera1Queue,
        camera1Output);
    ...
    if (pairOutputId == Pipe::InvalidFrameSyncOutputId ||
        ...
        std::cerr << "registerOutput failed: " << error.where << ": "
    ...
    if (!syncThread.start()) {
        const auto error = syncThread.lastError();
        std::cerr << "sync start failed: " << error.where << ": "
                  << error.message << std::endl;
        return 1;
    }
    if (!camera0.start()) {
        const auto error = camera0.lastError();
        std::cerr << "camera0 start failed: " << error.where << ": "
                  << error.message << std::endl;
    ...
    if (!camera1.start()) {
        ...
        std::cerr << "camera1 start failed: " << error.where << ": "
                  << error.message << std::endl;
        camera0.stopAndJoin();
        syncThread.stopAndJoin();
        return 1;
    ...
    std::cout << "running MultiCameraReadOnlySyncD3D11 for "
              << durationSeconds
              << " sec. device0=" << device0
              << " device1=" << device1
              << " sync=timestamp-nearest"
              << " timestampSource=" << ToString(timestampSource)
              << " maxDiffUs=" << maxDifferenceUs
              << std::endl;

    DrainStats stats;
    const auto start = Clock::now();
    auto nextReport = start + std::chrono::milliseconds(reportMilliseconds);
    const auto stopTime = start +
        std::chrono::seconds(std::max(1, durationSeconds));
    ...
    while (Clock::now() < stopTime) {
        ...
        if (now >= nextReport) {
            ...
            nextReport = now +
                std::chrono::milliseconds(reportMilliseconds);
        }
    ...
    std::cout << "sync inputFrames=" << syncStats.inputFrames
              ...
              << syncStats.totalOutputQueueDrops
              << std::endl;
