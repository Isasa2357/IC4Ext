    ...
    const std::string& name)
{
    ...
    fatal.set(ErrorText(name + " readback initialization failed", readback.lastError()));
    return false;
}
...
{
    return std::thread([
        queue = std::move(queue), backend = std::move(backend),
        &display, &metrics, &fatal, &stopRequested, &measuring, options]() mutable {
        DedicatedReadback readback0;
        DedicatedReadback readback1;
        if (!InitializeReadback(readback0, backend, fatal, "pair-display/0") ||
            !InitializeReadback(readback1, backend, fatal, "pair-display/1")) return;
        ...
        try {
            RunLatestLoop(queue, fatal, stopRequested, [&](const auto& set) {
                ...
                if (!ReadBgr(readback0, RequireFrame(set, 0), options.readbackTimeoutMs,
                             cpu0, fatal, "pair-display/0") ||
                    !ReadBgr(readback1, RequireFrame(set, 1), options.readbackTimeoutMs,
                             cpu1, fatal, "pair-display/1")) {
                    if (active) metrics.recordFailure(true, false);
                    return;
                }
                cv::Mat image = JoinHorizontal(CpuFrameToBgr(cpu0), CpuFrameToBgr(cpu1));
                DrawLabel(image, "Pipeline 1: latest pair display");
                image = ResizeForDisplay(
                    image, options.displayMaximumWidth, options.displayMaximumHeight);
...
{
    return std::thread([
        pipelineName = std::move(pipelineName), cameraId,
        queue = std::move(queue), backend = std::move(backend),
        &display, &metrics, &fatal, &stopRequested, &measuring, options]() mutable {
        DedicatedReadback readback;
        ...
        try {
            RunLatestLoop(queue, fatal, stopRequested, [&](const auto& set) {
                ...
                const auto started = Clock::now();
                IC4Ext::CpuFrame cpu;
                if (!ReadBgr(readback, RequireFrame(set, cameraId), options.readbackTimeoutMs,
                             cpu, fatal, pipelineName)) {
                    if (active) metrics.recordFailure(true, false);
                    return;
                }
    ...
    WorkerOptions options)
{
    return std::thread([
        queue = std::move(queue), backend = std::move(backend),
        &metrics, &fatal, &measuring, options]() mutable {
        DedicatedReadback readback0;
        DedicatedReadback readback1;
        if (!InitializeReadback(readback0, backend, fatal, "pair-video/0") ||
            !InitializeReadback(readback1, backend, fatal, "pair-video/1")) return;
        cv::VideoWriter writer;
        try {
            RunAllFrameLoop(queue, fatal, [&](const auto& set) {
                ...
                IC4Ext::CpuFrame cpu1;
                if (!ReadBgr(readback0, RequireFrame(set, 0), options.readbackTimeoutMs,
                             cpu0, fatal, "pair-video/0") ||
                    !ReadBgr(readback1, RequireFrame(set, 1), options.readbackTimeoutMs,
                             cpu1, fatal, "pair-video/1")) {
                    if (active) metrics.recordFailure(true, false);
                    return;
                }
                cv::Mat joined = JoinHorizontal(CpuFrameToBgr(cpu0), CpuFrameToBgr(cpu1));
                if (!writer.isOpened() && !OpenWriter(
                        writer, options.outputDirectory / "pair.avi", joined.size(),
                        options, metrics, fatal, "pair-video")) return;
                writer.write(joined);
    ...
    WorkerOptions options)
{
    return std::thread([
        ...
        backend = std::move(backend), &metrics, &fatal, &measuring, options]() mutable {
        DedicatedReadback readback;
        if (!InitializeReadback(readback, backend, fatal, pipelineName)) return;
        cv::VideoWriter writer;
        try {
            RunAllFrameLoop(queue, fatal, [&](const auto& set) {
                ...
                cv::Mat image = CpuFrameToBgr(cpu);
                if (!writer.isOpened() && !OpenWriter(
                        writer, options.outputDirectory / fileName, image.size(),
                        options, metrics, fatal, pipelineName)) return;
                writer.write(image);
    ...
    std::atomic<bool>& measuring)
{
    return std::thread([
        queue = std::move(queue), backend = std::move(backend),
        &metrics, &fatal, &measuring]() mutable {
        SobelProcessor processor;
        if (!processor.initialize(backend)) {
            fatal.set(ErrorText("HLSL Sobel initialization failed", processor.lastError()));
            return;
        }
        try {
            RunAllFrameLoop(queue, fatal, [&](const auto& set) {
                ...
                if (!processor.process(RequireFrame(set, 0)) ||
                    ...
                    fatal.set(ErrorText("HLSL Sobel failed", processor.lastError()));
                    return;
                }
    ...
    return std::thread([
        ...
        try {
            RunAllFrameLoop(queue, fatal, [&](const auto& set) {
                ...
                const auto started = Clock::now();
                IC4Ext::CpuFrame cpu;
                if (!ReadBgr(readback, RequireFrame(set, 0), options.readbackTimeoutMs,
                             cpu, fatal, "opencv-canny")) {
                    if (active) metrics.recordFailure(true, false);
                    return;
                }
    ...
    return std::thread([
        ...
        try {
            RunAllFrameLoop(queue, fatal, [&](const auto& set) {
                ...
                const auto started = Clock::now();
                IC4Ext::CpuFrame cpu;
                if (!ReadBgr(readback, RequireFrame(set, 1), options.readbackTimeoutMs,
                             cpu, fatal, "opencv-sobel")) {
                    if (active) metrics.recordFailure(true, false);
                    return;
                }
    ...
    WorkerOptions options)
{
    return std::thread([
        ...
        if (!InitializeReadback(readback0, backend, fatal, "opencv-pair/0") ||
            !InitializeReadback(readback1, backend, fatal, "opencv-pair/1")) return;
        ...
        try {
            RunLatestLoop(queue, fatal, stopRequested, [&](const auto& set) {
                ...
                IC4Ext::CpuFrame cpu1;
                if (!ReadBgr(readback0, RequireFrame(set, 0), options.readbackTimeoutMs,
                             cpu0, fatal, "opencv-pair/0") ||
                    !ReadBgr(readback1, RequireFrame(set, 1), options.readbackTimeoutMs,
                             cpu1, fatal, "opencv-pair/1")) {
                    if (active) metrics.recordFailure(true, false);
                    return;
                }
