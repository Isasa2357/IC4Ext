...
D3D12ReadOnlyFrame D3D12FrameWriter::publish(
    const ::IC4Ext::D3D12ReadyToken& ready,
    FrameTiming timing,
...
std::size_t D3D12FramePool::capacity() const
{
    return stats().capacity;
}

std::size_t D3D12FramePool::availableCount() const
{
    return stats().available;
}

std::size_t D3D12FramePool::inFlightCount() const
{
    return stats().inFlight();
}

bool D3D12FramePool::hasAvailableFrame() const
{
