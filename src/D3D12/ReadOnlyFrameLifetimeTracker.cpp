    ...
    const ::IC4Ext::D3D12ReadyToken& token) noexcept
{
    ...
    try {
        queue.GpuWait(token.fence.Get(), token.value);
        return true;
    } catch (...) {
        return false;
    }
    ...
    const ReadOnlyFrame& frame) noexcept
{
    ...
    return WaitForReadyTokenOnQueue(queue, frame.readyToken());
}
...
{
    if (!frame || !consumerCompletion.isValid()) {
        return false;
    }

    if (consumerCompletion.isReady()) {
        return true;
    }

    std::lock_guard<std::mutex> lock(mutex_);
    entries_.push_back(Entry{std::move(frame), std::move(consumerCompletion)});
    ++retainedTotal_;
    return true;
}
...
bool ReadOnlyFrameLifetimeTracker::retainUntil(
    ...
    ::IC4Ext::D3D12ReadyToken consumerCompletion)
{
    if (!frameSet || !consumerCompletion.isValid()) {
        return false;
    }
...
std::size_t ReadOnlyFrameLifetimeTracker::collectCompleted()
{
    ...
    entries_.erase(
        std::remove_if(
            ...
            entries_.end(),
            [](const Entry& entry) {
                return entry.completion.isReady();
            }),
        entries_.end());
...
bool ReadOnlyFrameLifetimeTracker::waitAllAndClear(std::uint32_t timeoutMs) noexcept
{
    std::lock_guard<std::mutex> lock(mutex_);
    for (const auto& entry : entries_) {
        if (!entry.completion.wait(timeoutMs)) {
            return false;
        }
    }
    collectedTotal_ += static_cast<std::uint64_t>(entries_.size());
