...
std::optional<Json> LoadJsonFile(const std::filesystem::path& path, ErrorInfo& outError)
{
    try {
        ...
        outError = MakeError(ErrorCode::InvalidArgument,
                             "LoadJsonFile",
                             std::string("JSON parse/apply error: ") + exception.what());
        return std::nullopt;
    }
}
...
bool TryGetJsonInt(const Json& state, const char* name, int& output)
{
...
bool TryGetJsonDouble(const Json& state, const char* name, double& output)
{
    if (!state.is_object() || !state.contains(name)) return false;
    const auto& value = state.at(name);
...
bool TryGetJsonPixelFormat(const Json& state, CameraPixelFormat& output)
{
    if (!state.is_object() || !state.contains("PixelFormat") ||
        !state.at("PixelFormat").is_string()) {
        return false;
    }
    return ParseCameraPixelFormat(state.at("PixelFormat").get<std::string>(), output);
}
...
public:
    ...
    {
        std::lock_guard<std::mutex> lock(statsMutex);
        stats.droppedPendingBuffers += count;
    }
    ...
    bool applyJsonStateConfig()
    {
        ...
        auto properties = getPropertyMap("D3D12CameraCapture::applyJsonStateConfig / devicePropertyMap");
    ...
    {
        auto properties = getPropertyMap("D3D12CameraCapture::open / devicePropertyMap");
        if (!properties) return false;
        ic4::Error error;
        ...
        if (config.streamRequest.width > 0 &&
            ...
            setError(ErrorCode::IC4Error, "D3D12CameraCapture::open / Width", IC4ErrorMessage(error));
            return false;
        }
        if (config.streamRequest.height > 0 &&
            !properties->setValue(ic4::PropId::Height, config.streamRequest.height, error)) {
            setError(ErrorCode::IC4Error, "D3D12CameraCapture::open / Height", IC4ErrorMessage(error));
            return false;
        }
        if (config.streamRequest.offsetX &&
            !properties->setValue(ic4::PropId::OffsetX, *config.streamRequest.offsetX, error)) {
            setError(ErrorCode::IC4Error, "D3D12CameraCapture::open / OffsetX", IC4ErrorMessage(error));
            return false;
        }
        if (config.streamRequest.offsetY &&
            !properties->setValue(ic4::PropId::OffsetY, *config.streamRequest.offsetY, error)) {
            setError(ErrorCode::IC4Error, "D3D12CameraCapture::open / OffsetY", IC4ErrorMessage(error));
            return false;
        }
        if (config.streamRequest.fps > 0.0 &&
            !properties->setValue(ic4::PropId::AcquisitionFrameRate,
                                  config.streamRequest.fps,
                                  error)) {
            setError(ErrorCode::IC4Error,
                     "D3D12CameraCapture::open / AcquisitionFrameRate",
                     IC4ErrorMessage(error));
            return false;
        }
        return true;
    ...
    bool applyPropertyOverrides()
    {
        ...
        auto properties = getPropertyMap("D3D12CameraCapture::open / propertyOverrides");
        if (!properties) return false;
        for (const auto& overrideValue : config.propertyOverrides) {
            ...
            if (!ok) {
                setError(ErrorCode::IC4Error,
                         "D3D12CameraCapture::open / propertyOverride " + overrideValue.propertyName,
                         error.isError() ? IC4ErrorMessage(error)
                                         : "IC4 property setValue returned false");
                return false;
            }
...
void D3D12CameraCapture::setError(ErrorCode code,
                              ...
                              const D3D12BackendContext& backend)
{
    ...
    if (!impl_->device || !impl_->queue) {
        setError(ErrorCode::D3D12Error,
                 "D3D12CameraCapture::open",
                 "D3D12 backend has null resolved device/queue");
        return false;
    }
    ...
    if (!impl_->ic4Context->initialized()) {
        setError(ErrorCode::IC4Error,
                 "D3D12CameraCapture::open / initLibrary",
    ...
    if (error.isError() || !impl_->grabber || !(*impl_->grabber)) {
        setError(ErrorCode::IC4Error,
                 "D3D12CameraCapture::open / Grabber",
                 IC4ErrorMessage(error));
        return false;
    }
    if (!impl_->grabber->deviceOpen(*deviceInfo, error)) {
        setError(ErrorCode::IC4Error,
                 "D3D12CameraCapture::open / deviceOpen",
                 IC4ErrorMessage(error));
        return false;
    }
    ...
    if (error.isError() || !impl_->queueSink) {
        setError(ErrorCode::IC4Error,
                 "D3D12CameraCapture::open / QueueSink::create",
                 IC4ErrorMessage(error));
        close();
        return false;
    }
    ...
    if (!impl_->grabber->streamSetup(impl_->queueSink, setupOption, error)) {
        setError(ErrorCode::IC4Error,
                 "D3D12CameraCapture::open / streamSetup",
...
bool D3D12CameraCapture::startAcquisition()
{
    if (!opened_.load() || !impl_) {
        setError(ErrorCode::NotOpened,
                 "D3D12CameraCapture::startAcquisition",
...
bool D3D12CameraCapture::stopAcquisition()
{
    if (!opened_.load() || !impl_) {
        setError(ErrorCode::NotOpened,
                 "D3D12CameraCapture::stopAcquisition",
                 "Capture is not opened");
        return false;
    }
...
D3D12ReadResult D3D12CameraCapture::read(const CameraReadOptions& options)
{
    ...
    if (pointerError.isError() || !data) {
        result.error = MakeError(ErrorCode::IC4Error,
                                 "D3D12CameraCapture::read / ImageBuffer::ptr",
    ...
    if (sizeError.isError() || dataSize == 0) {
        result.error = MakeError(ErrorCode::IC4Error,
                                 "D3D12CameraCapture::read / ImageBuffer::bufferSize",
                                           ...
                                           bool applyNestedSelectorStates)
{
    if (!opened_.load() || !impl_) {
        setError(ErrorCode::NotOpened,
                 "D3D12CameraCapture::applyIC4StateJson",
                 "Capture is not opened");
        return false;
    }
...
{
    if (!opened_.load() || !impl_) {
        setError(ErrorCode::NotOpened, "D3D12CameraCapture::setIC4Property", "Capture is not opened");
        return false;
    }
    const bool ok = impl_->setPropertyValue(propertyName,
                                            value,
                                            "D3D12CameraCapture::setIC4Property(bool)");
    lastError_ = ok ? NoError() : impl_->lastError;
    return ok;
}

bool D3D12CameraCapture::setIC4Property(const std::string& propertyName, int value)
{
...
bool D3D12CameraCapture::setIC4Property(const std::string& propertyName, std::int64_t value)
{
    ...
    const bool ok = impl_->setPropertyValue(propertyName,
                                            value,
                                            "D3D12CameraCapture::setIC4Property(int64)");
    lastError_ = ok ? NoError() : impl_->lastError;
    return ok;
}
...
bool D3D12CameraCapture::setIC4Property(const std::string& propertyName, double value)
{
    ...
    const bool ok = impl_->setPropertyValue(propertyName,
                                            value,
                                            "D3D12CameraCapture::setIC4Property(double)");
    lastError_ = ok ? NoError() : impl_->lastError;
    return ok;
}

bool D3D12CameraCapture::setIC4Property(const std::string& propertyName, const char* value)
{
    return setIC4Property(propertyName, std::string(value ? value : ""));
}

bool D3D12CameraCapture::setIC4Property(const std::string& propertyName,
                                        const std::string& value)
{
    ...
    const bool ok = impl_->setPropertyValue(propertyName,
                                            value,
                                            "D3D12CameraCapture::setIC4Property(string)");
    lastError_ = ok ? NoError() : impl_->lastError;
    return ok;
}
...
bool D3D12CameraCapture::setPixelFormat(CameraPixelFormat format)
{
    if (!IsSupportedConversion(format,
        ...
        setError(ErrorCode::UnsupportedFormat,
                 "D3D12CameraCapture::setPixelFormat",
