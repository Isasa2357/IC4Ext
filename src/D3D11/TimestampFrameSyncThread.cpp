...
bool PushSucceeded(ThreadKit::Queues::QueuePushResult result) noexcept
{
    return result == ThreadKit::Queues::QueuePushResult::Pushed ||
           result == ThreadKit::Queues::QueuePushResult::DroppedOldestAndPushed;
}

bool PushDropped(ThreadKit::Queues::QueuePushResult result) noexcept
{
    return result != ThreadKit::Queues::QueuePushResult::Pushed;
}
...
struct OutputEntry
{
    ...
    std::shared_ptr<D3D11ReadOnlyFrameSetQueue> queue;
...
class D3D11FrameSyncThread::Impl
{
public:
    Impl(
        std::shared_ptr<D3D11IndexedReadOnlyFrameQueue> input,
    ...
    {
        CameraId cameraId = 0;
        std::deque<D3D11IndexedReadOnlyCameraFrame> frames;
    };
    ...
    struct CompleteFrameSet
    {
        ...
        std::vector<D3D11IndexedReadOnlyFrame> frames;
    };
    ...
    bool start()
    {
        ...
        if (!configValue.isValid()) {
            setError(
                ErrorCode::InvalidArgument,
                "D3D11FrameSyncThread::start",
                "Invalid timestamp synchronization configuration");
            return false;
        }
        ...
        FrameSyncOutputConfig config)
    {
        if (!queue) {
            setError(
                ErrorCode::InvalidArgument,
                "D3D11FrameSyncThread::registerOutput",
    ...
    bool updateOutput(FrameSyncOutputId id, FrameSyncOutputConfig config)
    {
        if (!validateOutputConfig(
                config,
                "D3D11FrameSyncThread::updateOutput")) {
            return false;
        }
    ...
    bool stopOutputSupply(FrameSyncOutputId id)
    {
        ...
        if (!entry) {
            setError(
                ErrorCode::InvalidArgument,
                "D3D11FrameSyncThread::stopOutputSupply",
                "Output ID was not found");
            return false;
        }
    ...
    bool closeOutputChannel(FrameSyncOutputId id)
    {
        ...
        if (iterator == outputTable.end()) {
            setError(
                ErrorCode::InvalidArgument,
                "D3D11FrameSyncThread::closeOutputChannel",
                "Output ID was not found");
            return false;
        }
...
private:
    ...
    void workerLoop() noexcept
    {
        try {
            ...
            setError(
                ErrorCode::ThreadError,
                "D3D11FrameSyncThread::workerLoop",
                exception.what());
        } catch (...) {
            setError(
                ErrorCode::ThreadError,
    ...
    void handleInputFrame(D3D11IndexedReadOnlyCameraFrame&& frame)
    {
    ...
    void emitFrontSet()
    {
        ...
        for (auto& buffer : buffers) {
            reference = std::max(
                reference,
                syncTimestampNs(buffer.frames.front()));
            complete.frames.push_back(D3D11IndexedReadOnlyFrame{
                buffer.cameraId,
    ...
    void dispatchCompleteSet(const CompleteFrameSet& complete)
    {
        ...
        for (const auto& output : outputTable) {
            ...
            try {
                D3D11ReadOnlyFrameSet::FrameList selected;
                ...
                auto frameSet = D3D11ReadOnlyFrameSet::Create(
                ...
                if (result == ThreadKit::Queues::QueuePushResult::Closed) {
                    ...
                    faultOutput(
                        *output,
                        "D3D11FrameSyncThread::dispatchCompleteSet",
    ...
    std::uint64_t syncTimestampNs(
        const D3D11IndexedReadOnlyCameraFrame& frame) const noexcept
    {
...
D3D11FrameSyncThread::~D3D11FrameSyncThread() = default;
bool D3D11FrameSyncThread::start() { return impl_->start(); }
void D3D11FrameSyncThread::requestStop() { impl_->requestStop(); }
void D3D11FrameSyncThread::join() { impl_->join(); }
void D3D11FrameSyncThread::stopAndJoin() { impl_->stopAndJoin(); }
bool D3D11FrameSyncThread::isRunning() const noexcept { return impl_->isRunning(); }

FrameSyncOutputId D3D11FrameSyncThread::registerOutput(
    std::shared_ptr<D3D11ReadOnlyFrameSetQueue> outputQueue,
    FrameSyncOutputConfig config)
{
...
bool D3D11FrameSyncThread::updateOutput(
    FrameSyncOutputId outputId,
    FrameSyncOutputConfig config)
{
...
bool D3D11FrameSyncThread::stopOutputSupply(FrameSyncOutputId outputId)
{
    return impl_->stopOutputSupply(outputId);
}

bool D3D11FrameSyncThread::closeOutputChannel(FrameSyncOutputId outputId)
{
    return impl_->closeOutputChannel(outputId);
}

std::optional<FrameSyncOutputConfig> D3D11FrameSyncThread::outputConfig(
    FrameSyncOutputId outputId) const
{
    return impl_->outputConfig(outputId);
}

std::optional<FrameSyncOutputState> D3D11FrameSyncThread::outputState(
    FrameSyncOutputId outputId) const
{
    return impl_->outputState(outputId);
}

std::vector<FrameSyncOutputInfo> D3D11FrameSyncThread::outputs() const
{
    return impl_->outputs();
}

std::optional<FrameSyncOutputStats> D3D11FrameSyncThread::outputStats(
    FrameSyncOutputId outputId) const
{
    return impl_->outputStats(outputId);
}

std::optional<ErrorInfo> D3D11FrameSyncThread::outputLastError(
    FrameSyncOutputId outputId) const
{
    return impl_->outputLastError(outputId);
}

const FrameSyncConfig& D3D11FrameSyncThread::config() const noexcept
{
    return impl_->configValue;
}

FrameSyncStats D3D11FrameSyncThread::stats() const { return impl_->stats(); }
ErrorInfo D3D11FrameSyncThread::lastError() const { return impl_->lastError(); }
